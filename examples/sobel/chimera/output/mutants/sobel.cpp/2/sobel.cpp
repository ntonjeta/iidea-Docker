...
void filter (boost::shared_ptr<Image> srcImagePtr,boost::shared_ptr<Image> dstImagePtr){
  ...
  for( x = 0 ; x < srcImagePtr->width ; x = x + stride1 ) {
  ...
  for (y = 1 ; y < (srcImagePtr->height - 1) ; y = y + stride2) {
    x = 0 ;
    HALF_WINDOW(srcImagePtr, x, y, w);
    ...
    for( x = 1 ; x < srcImagePtr->width - 1 ; x = x + stride3 ) {
  ...
  for(x = 0 ; x < srcImagePtr->width ; x = x + stride4) {
    HALF_WINDOW(srcImagePtr, x, y, w) ;
