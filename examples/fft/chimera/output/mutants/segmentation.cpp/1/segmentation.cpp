...
int initClusters(Clusters* clusters, int k, float scale) {
  ...
  for (i = 0; i < clusters->k; i ++) {
    x = ((float)(::fap::FloatingPointType((float) ((float)rand()), OP_1))/RAND_MAX) * (float)(::fap::FloatingPointType((float) scale, OP_0));
    clusters->centroids[i].r = x;

    x = ((float)(::fap::FloatingPointType((float) ((float)rand()), OP_3))/RAND_MAX) * (float)(::fap::FloatingPointType((float) scale, OP_2));
    clusters->centroids[i].g = x;

    x = ((float)(::fap::FloatingPointType((float) ((float)rand()), OP_5))/RAND_MAX) * (float)(::fap::FloatingPointType((float) scale, OP_4));
